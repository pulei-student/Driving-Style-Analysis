...
def plotting_style(all_counts):
    ...
    for idx in range(len(all_counts)): 
        ...
        axes[-1].set_ylabel('加速度'+r'$a_x$',size=20,fontproperties=zhfont1)
        axes[-1].set_xticks(np.linspace(0.5,4.5,5,endpoint=True))
        axes[-1].set_xticklabels(X,fontproperties=zhfont1,size=18)
        axes[-1].set_xlabel('相对速度'+r'$\Delta$v',size=20,fontproperties=zhfont1)
        axes[-1].set_yticks(np.linspace(0.5,4.5,5,endpoint=True))
        axes[-1].set_yticklabels(Y,fontproperties=zhfont1,size=18)
        axes[-1].set_title(titles[idx],fontproperties=zhfont1,size=20)
